...
contract TobuyDebotShop is TobuyDebot{

    uint32 m_buyId;    // Buy id for mark
    ...
    function getDebotInfo() virtual public functionID(0xDEB) override view returns(
        ...
        name = "TOBUY DeBot SHOP";
        version = "0.2.0";
        publisher = "TON Labs";
        key = "TOBUY list manager";
        author = "TON Labs";
        support = address.makeAddrStd(0, 0x61e01d6df5a8d7677d9ab2daf7f258f1e2a7fe73da5320300395f99e01dc3b5f);
        hello = "Hi, i'm a TOBUY DeBot. Let's go to shopping!";
        language = "en";
        dabi = m_debotAbi.get();
        icon = m_icon;
    }
    ...
    function _menu() public override {
        ...
        Menu.select(
            ...
            sep,
            [
                MenuItem("Mark item","",tvm.functionId(markBuy)),
    ...
    function markBuy(uint32 index) public {
        index = index;
        if (m_purchase.unpaidPurchase> 0) {
            Terminal.input(tvm.functionId(markBuy_), "Enter item number:", false);
        } else {
            Terminal.print(0, "All items from TOBUY list are purchased");
            _menu();
        }
    }

    function markBuy_(string value) public {
        (uint256 num,) = stoi(value);
        m_buyId = uint32(num);
        Terminal.input(tvm.functionId(markBuy__), "Enter price of item:", false);
    }

    function markBuy__(string value) public view {
        (uint256 num,) = stoi(value);
        optional(uint256) pubkey = 0;
        ITobuy(m_address).markBuy{
                abiVer: 2,
                extMsg: true,
                sign: true,
                pubkey: pubkey,
